...
void get_result(int st, int ed, int idx, int right, int target) {
	if (st >= right) {
		return;
	}
	if (ed < right) {
		result += std::lower_bound(segment[idx].begin(), segment[idx].end(), target) - segment[idx].begin();
		return;
	}
	int mid = (st + ed) / 2;
	get_result(st, mid, 2 * idx, right, target);
	get_result(mid + 1, ed, 2 * idx + 1, right, target);
	return;
}
